...
export function ArtistBubble({ artist, size = "md" }: { artist: Artist; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <Link href={`/artistas/${artist.slug}`} className="group flex flex-col items-center gap-3 text-center">
      ...
      <div>
        <div className="font-bold leading-tight"><AutoTranslate text={artist.name} /></div>
        <div className="font-mono text-[10px] tracking-widest text-muted-foreground"><AutoTranslate text={artist.genre} /></div>
      </div>
    </Link>
  );
}
